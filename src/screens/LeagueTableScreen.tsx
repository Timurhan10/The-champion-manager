...
export default function LeagueTableScreen() {
  ...
  const [viewTab, setViewTab] = useState<ViewTab>('standings');
  ...
  const topScorers = useMemo(() => {
    return Object.values(players)
      .filter(p => p.seasonStats.goals > 0 && p.teamId)
  ...
  const topAssisters = useMemo(() => {
    return Object.values(players)
      .filter(p => p.seasonStats.assists > 0 && p.teamId)
                ...
                <tbody>
                  {standings.map((s, i) => {
                    ...
                    return (
                      <tr
                        key={s.teamId}
                        className={`border-b border-gray-700/50 cursor-pointer ${isUser ? 'bg-emerald-900/30' : 'hover:bg-gray-700/30'} ${
                          isTopZone ? 'border-l-4 border-l-green-500' : isBottomZone ? 'border-l-4 border-l-red-500' : ''
                        }`}
                        onClick={() => viewTeam(s.teamId)}
                      >
                        ...
                        <td className={`p-2 font-medium ${isUser ? 'text-emerald-400' : ''}`}>
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="truncate hover:underline">{team?.shortName ?? '?'}</span>
                          </div>
                        </td>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-800/80 backdrop-blur rounded-lg p-3">
            ...
            <div className="space-y-1">
              ...
              {topScorers.map((p, i) => {
                const team = p.teamId ? teams[p.teamId] : null;
                return (
                  <div key={p.id} className="flex items-center gap-2 text-xs p-1.5 bg-gray-700/50 rounded">
                    <span className="text-gray-400 w-5 text-right">{i + 1}.</span>
                    {team && <TeamEmblem shortName={team.shortName} primary={team.colors.primary} secondary={team.colors.secondary} size={14} />}
                    <span className="text-white flex-1 truncate">{p.name}</span>
                    <span className="text-emerald-400 font-bold">{p.seasonStats.goals}</span>
                  </div>
                );
              })}
            </div>
              ...
              {topAssisters.map((p, i) => {
                ...
                return (
                  <div key={p.id} className="flex items-center gap-2 text-xs p-1.5 bg-gray-700/50 rounded">
                    ...
                    {team && <TeamEmblem shortName={team.shortName} primary={team.colors.primary} secondary={team.colors.secondary} size={14} />}
                    <span className="text-white flex-1 truncate">{p.name}</span>
                    <span className="text-blue-400 font-bold">{p.seasonStats.assists}</span>
                  </div>
                );
              })}
            </div>
